...
include_once '../../../includes/db_connect.php';
		...
		if ($FirstDay + $DaysInMonth > 35) { $rowAmnt = 6;
			...
			$taskQuery = "SELECT evntdate, title, start, end, task FROM schedule WHERE evntdate >= '".$evntStart."' AND evntdate <= '".$evntEnd."'";
			$evntQuery = $mysqli->prepare($taskQuery);
			$evntQuery->execute();
			$evntQuery->bind_result($aDate, $scheduleT, $startT, $endT, $taskS);
			while ($evntQuery->fetch()) {
			$countRow++;
			$aDate = explode('-', $aDate);
			$aDate = $aDate[2];
			$datevar.= $aDate.",";
			if (array_key_exists($aDate, $tArray)) { $tArray[$aDate].= ",".$scheduleT."|".$startT."|".$endT."|".$taskS; }
			else { $tArray[$aDate] = $scheduleT."|".$startT."|".$endT."|".$taskS; }
			}
			$evntQuery->close();
		$titlevar = ''; $startvar = ''; $endvar = ''; $taskvar = ''; $checkZero = 0;
		for ($i = 0; $i < $rowAmnt; $i++) {
			$TblRows .= '<tr>';
		for ($k = 0; $k < 7; $k++) {
		...
		$taskvar = substr($taskvar, 0, -1);
		$datevar = substr($datevar, 0, -1);
		if ($checkZero > 1) { $titlevar = '['.$titlevar.']'; $startvar = '['.$startvar.']'; $endvar = '['.$endvar.']'; $taskvar = '['.$taskvar.']'; $datevar = '['.$datevar.']'; }
		$titlevar = "var titleArr = ".$titlevar.";";
		$startvar = "var startArr = ".$startvar.";";
		$endvar = "var endArr = ".$endvar.";";
		$taskvar = "var taskArr = ".$taskvar.";";
